...
async def reset_user_on_error(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = "❌ একটি সমস্যা হয়েছে। সেশনটি রিসেট করা হয়েছে।\n\nআবার শুরু করতে নিচের বাটন থেকে আপনার কাজ বেছে নিন:"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message:
        try:
            ...
            pass
        msg = await context.bot.send_message(
            chat_id=user_id, text=welcome_text, reply_markup=get_main_menu(), parse_mode='Markdown'
        )
        user_data[user_id] = {'status_message_id': msg.message_id, 'last_activity': datetime.now()}
    elif update.callback_query:
        await update.callback_query.answer()
        await safe_edit_message(context, user_id, update.callback_query.message.message_id, welcome_text, get_main_menu())
        user_data[user_id] = {'status_message_id': update.callback_query.message.message_id, 'last_activity': datetime.now()}
...
async def start_merge(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_data[user_id].update({
        'mode': 'merge', 'audio_files': [], 'audio_names': [],
        'status_message_id': update.callback_query.message.message_id, 'last_activity': datetime.now()
    })
...
async def add_more_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'merged_file' not in user_data.get(user_id, {}):
        ...
        return
    user_data[user_id].update({
        'mode': 'add_more', 'new_audio_files': [], 'new_audio_names': [],
        'status_message_id': update.callback_query.message.message_id, 'last_activity': datetime.now()
    })
...
async def start_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user_data[user_id].update({
        'mode': 'video', 'image': None, 'audio': None,
        'status_message_id': update.callback_query.message.message_id, 'last_activity': datetime.now()
    })
...
async def cancel_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit_message(context, user_id, query.message.message_id, welcome_text, get_main_menu())

    # Reset the user's state in the dictionary, keeping only necessary info.
    user_data[user_id] = {
...
async def merge_audios(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        user_data[user_id].update({
            'merged_file': output_path, 'file_ready': True, 'last_activity': datetime.now()
        })
    except Exception as e:
...
async def download_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with open(output_path, 'rb') as audio_file:
            await context.bot.send_audio(chat_id=user_id, audio=audio_file, title="Merged Audio", caption=f"✅ আপনার অডিও ফাইল প্রস্তুত!")
        text = "এখন কি করবেন?"
...
async def post_init(application: Application):
    """Starts background tasks after the bot is initialized."""
