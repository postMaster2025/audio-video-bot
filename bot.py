...
async def merge_audios(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Merges multiple audio files into one with optimized file size."""
    user_id = update.effective_user.id
    ...
    try:
        ...
        combined.export(output_path, format="mp3", bitrate="128k", parameters=["-ac", "1"])  # Mono for smaller size
        
        progress = 100
        text = f"✅ *মার্জ সম্পন্ন!*\n\n{get_progress_bar(progress)} 100%\n\nফাইল প্রস্তুত! ডাউনলোড করতে নিচের বাটনে ক্লিক করুন।"
...
async def create_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Creates a video from an image and audio with optimized file size."""
    user_id = update.effective_user.id
    status_id = user_data[user_id]['status_message_id']
    ...
    try:
        ...
        # Optimized FFmpeg command for smaller file size
        cmd = [
            'ffmpeg', '-loop', '1', '-i', image_path, '-i', audio_path,
            '-c:v', 'libx264', '-preset', 'fast', '-crf', '32',  # Higher CRF for smaller size
            '-c:a', 'aac', '-b:a', '96k', '-ac', '1',  # Lower bitrate, mono audio
            '-pix_fmt', 'yuv420p', '-shortest', '-movflags', '+faststart',
            '-vf', 'scale=640:480',  # Scale down resolution
